import {_upsertAdministrator} from './_operations'
...
const post = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    const administrator = await _upsertAdministrator(upsertArgs)
    
    return res.status(200).json({
      message: 'Administrator upserted.',
      data: administrator
    })
  } catch (error) {
    console.error("[api] administrator/upsert", error)
