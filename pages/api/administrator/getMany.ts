import {_getManyAdministrator} from './_operations'
...
const post = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    const administrators = await _getManyAdministrator(findManyArgs)

    return res.status(200).json({data: administrators})
  } catch (error) {
    console.error("[api] administrator/getMany", error)
